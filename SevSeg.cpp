...
void SevSeg::begin(uint8_t numDigitsIn, const uint8_t pin, bool updateWithDelaysIn, bool leadingZerosIn, bool disableDecPoint) {

  pixels = Adafruit_NeoPixel(7 * numDigitsIn, pin, NEO_GRB + NEO_KHZ800);
  pixels.begin();
  ...
  for (uint8_t digitNum = 0 ; digitNum < numDigits ; digitNum++) {
    ...
    for (uint8_t segmentNum = 0 ; segmentNum < 7 ; segmentNum++) {
      if (digitCodes[digitNum] & (1 << segmentNum)) {
        pixels.setPixelColor(segmentNum + (numDigits - digitNum - 1) * 7, color);
      } else {
        pixels.setPixelColor(segmentNum + (numDigits - digitNum - 1) * 7, bkgColor);
      }
    }
  }
...
// Toggle period whereever
void SevSeg::togglePeriod(int8_t position) {
  if(position >= 0 && position < MAXNUMDIGITS) {
    digitCodes[position] ^= digitCodeMap[PERIOD_IDX];
  }
}
...
void SevSeg::setColor(uint32_t newColor) {
  color = newColor;
}


void SevSeg::setBkgColor(uint32_t newColor) {
  bkgColor = newColor;
}
